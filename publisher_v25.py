...
def ar(text: Optional[str]) -> str:
    ...
    v26 النهائي - استراتيجية ذكية:
    - إذا libraqm متاح: نمرر النص خاماً (Pillow يعالج shaping + bidi تلقائياً)
    - إذا libraqm غير متاح لكن RTL متاح: نطبق reshape + bidi يدوياً
    - في الحالات الأخرى: نمرر النص خاماً (آخر حل)

    هذه الاستراتيجية تضمن العمل في كل البيئات.
    """
    if not text:
        return ""

    # الأفضلية الأولى: libraqm (يعالج العربية تلقائياً)
    if _HAS_RAQM:
        return str(text)

    # الأفضلية الثانية: reshape + bidi يدوي
    if _HAS_RTL:
        try:
...
FONT_PATHS: Tuple[str, ...] = (
    # ============================================
    # v26 الإصدار النهائي: Noto Sans Arabic أولاً
    # لأنه يدعم العربية + الإنجليزية + الأرقام + الرموز
    # في نفس الخط (لا مربعات للأحرف الإنجليزية)
    # ============================================
    "/usr/share/fonts/truetype/noto/NotoSansArabic-Bold.ttf",
    "/usr/share/fonts/truetype/noto/NotoNaskhArabic-Bold.ttf",
    "/usr/share/fonts/truetype/noto/NotoNaskhArabicUI-Bold.ttf",
    "/usr/share/fonts/truetype/noto/NotoKufiArabic-Bold.ttf",
    # Amiri خط جميل لكن لا يدعم اللاتينية بالكامل
...
def get_font(size: int) -> ImageFont.ImageFont:
    ...
    v26: يستخدم النظام الحديث في Pillow اللي يدعم fallback fonts،
    بحيث الحروف اللي ما يدعمها الخط الأساسي تُرسم من خط احتياطي.
    """
...
def _clean_text_for_font(text: str) -> str:
    """تنظيف النص من الأحرف اللاتينية التي قد تظهر كمربعات.

    v26: نستبدل الأحرف الإنجليزية بمكافئها العربي أو نزيلها
    من العنوان فقط (الكابشن يبقى كما هو).
    """
    if not text:
        return ""
    # استبدالات شائعة في الأخبار العراقية
    replacements = {
        '"': '"',  # علامة تنصيص يسرى
        '"': '"',  # علامة تنصيص يمنى
        ''': "'",
...
NEWS_SOURCES: Tuple[str, ...] = (
    ...
    'https://imn.iq/ar/feed/',                     # شبكة الإعلام العراقي
    'https://www.iraqakhbar.com/feed/',            # أخبار العراق
    'https://iraqpaper.com/feed/',                 # جريدة العراق الإخبارية
    'https://www.faceiraq.org/feed/',              # موسوعة العراق
    'https://ninanews.com/feed',                   # النينا
    'https://www.al-furatnews.com/feed/',          # الفرات نيوز

    # ============================================
    # المصادر الإقليمية والعربية (15 مصدر)
    # ============================================
    ...
    'https://www.alarabiya.net/.mrss/ar/arab-and-world/iraq.xml',       # العربية - العراق
    'https://www.skynewsarabia.com/web/feeds/rss/middle-east.xml',      # سكاي نيوز عربية
    'https://www.france24.com/ar/rss',                                  # France 24 عربي
    'https://rss.dw.com/rdf/rss-ar-all',                                # DW عربي
    'https://al-ain.com/rss',                                           # العين الإخبارية
    'https://arabic.cnn.com/api/v1/rss/middle-east/rss.xml',            # CNN عربي - الشرق الأوسط
    'https://arabic.cnn.com/api/v1/rss/world/rss.xml',                  # CNN عربي - العالم
    'https://aawsat.com/feed',                                          # الشرق الأوسط
    'https://www.eremnews.com/feed',                                    # إرم نيوز
    'https://24.ae/feed',                                               # 24 الإماراتية
)
