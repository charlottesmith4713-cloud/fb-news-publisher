...
def ar(text: Optional[str]) -> str:
    ...
    v26 (الإصدار النهائي المُصلَح): Pillow الجديد + libraqm في GitHub Actions
    يعالج النص العربي تلقائياً (shaping + bidi). لذلك نُعيد النص كما هو
    لتفادي المعالجة المزدوجة (double-processing) التي تسبب حروف منفصلة.
    """
    if not text:
        return ""
    return str(text)
