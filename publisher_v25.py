# -*- coding: utf-8 -*-
"""
نظام نشر الأخبار المستمر - النسخة 26 (Pro Design + Stable)
================================================================
✨ الجديد في v26:
  ✅ تصميم احترافي: خبر عاجل + شعار + تاريخ
  ✅ خطوط Amiri/Noto للعربية الأنيقة
  ✅ libraqm-aware (لا عكس مزدوج)
  ✅ تصفية الأخبار بدون صورة (REQUIRE_IMAGE)
  ✅ كشف تكرار محسّن للنصوص العربية
  ✅ 21 مصدر إخباري (15 + 6 جديد)
"""
...
# v26: التحقق من libraqm لتفادي العكس المزدوج
try:
...
def ar(text):
    ...
    v26: إذا libraqm متاح، لا نطبّق reshape/bidi
    لأن libraqm يعالج العربية تلقائياً (تفادي العكس المزدوج).
    """
    if not text:
        return ""
    if _HAS_RAQM:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if 'id' in result:
                ...
                logger.warning(f"⚠️ {page_name}: رد فيسبوك غير متوقع")
                return False
        else:
            try:
                logger.warning(
...
def main_loop(once=False, max_posts=None):
    ...
    logger.info(f"📊 ملخص: {posts_count} منشور | {cycle * LOOP_DELAY_SECONDS:.1f}s")
...
def verify_sources():
    ...
    logger.info("=" * 60)
    logger.info(f"📊 النتيجة: {working}/{len(NEWS_SOURCES)} مصدر يعمل | {total_iraqi} خبر عراقي")


def show_startup_banner(once=False, verify_only=False):
    logger.info("=" * 60)
    logger.info("🚀 نظام نشر الأخبار - النسخة 26 (Pro Design)")
    if verify_only:
    ...
    logger.info(f"🖼️  تصفية الصور: {'مُفعّلة' if REQUIRE_IMAGE else 'مُعطّلة'}")
    logger.info("=" * 60)


def main():
    parser = argparse.ArgumentParser(description='نظام نشر الأخبار العراقية v26')
