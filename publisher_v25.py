# -*- coding: utf-8 -*-
"""نظام نشر الأخبار - النسخة 26 (Pro Design)"""
...
NEWS_SOURCES = (
    ...
    'https://www.alarabiya.net/.mrss/ar/aswaq.xml',
    'https://www.skynewsarabia.com/web/feeds/rss/middle-east.xml',
    'https://www.france24.com/ar/rss',
    'https://rss.dw.com/rdf/rss-ar-all',
    'https://al-ain.com/rss',
)

IRAQ_KEYWORDS = (
    'العراق', 'عراق', 'بغداد', 'البصرة', 'الموصل', 'أربيل', 'كركوك',
    'السليمانية', 'النجف', 'كربلاء', 'الكاظمي', 'السوداني',
    'الحكومة العراقية', 'البرلمان العراقي', 'الجيش العراقي',
    'الحشد الشعبي', 'الكرد', 'العراقي', 'العراقية', 'العراقيين',
    'دينار', 'نفط العراق',
)
...
def _signal_handler(signum, _frame):
    ...
    logger.info(f"إيقاف لطيف - إشارة {signum}")
...
def cleanup_old_news(days=NEWS_CLEANUP_AFTER_DAYS):
    try:
        ...
        conn.close()
        if deleted > 0:
            logger.info(f"حذف {deleted} خبر قديم")
    except sqlite3.Error:
        pass
        ...
        for fut in as_completed(futures):
            try:
                ...
                if status == 'ok':
                    ...
                    total_saved += saved
                    if saved > 0:
                        logger.info(f"  {short}: {saved} جديد ({iraqi}/{total})")
            except Exception:
                pass
    if total_saved > 0:
        logger.info(f"إجمالي: {total_saved} خبر من {sources_ok}/{sources_total}")
    return total_saved
...
def _draw_calendar_icon(draw, x, y, size, color):
    ...
    draw.line([(x, y + 12), (x + size, y + 12)], fill=color, width=2)
...
def create_post_image(title, image_url, page_config):
    try:
        ...
        if news_image:
            ...
            news_resized = fast_gradient(iw, ih, p['bar_grad_end'], p['overlay_color'], vertical=True)
        ...
        for persian_path in (
            ...
            if os.path.exists(persian_path):
                try:
                    font_logo = ImageFont.truetype(persian_path, 44)
                    break
                except (OSError, IOError):
                    continue
        ...
        for dx, dy in [(-2, -2), (2, -2), (-2, 2), (2, 2)]:
            draw.text((pn_x + dx, pn_y + dy), page_name_ar, font=font_page, fill=p['glow'])
        draw.text((pn_x, pn_y), page_name_ar, font=font_page, fill=p['bar_text'])
        ...
        logger.error(f"خطأ create_post_image: {e}")
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.warning(f"فيسبوك رفض {page_name}: {resp.status_code} - {resp.text[:200]}")
            return False
    except requests.RequestException as e:
        logger.warning(f"خطأ شبكة {page_key}: {e}")
        return False
    except Exception as e:
        logger.error(f"خطأ post_to_facebook {page_key}: {e}")
        return False
...
def main_loop(once=False, max_posts=None):
    ...
    while not _shutdown_requested:
        ...
        try:
            ...
            news = get_unposted()
            if news:
                logger.info(f"نشر: \"{news['title'][:60]}...\" (محاولة {news['retry_count'] + 1}/{MAX_RETRIES_PER_NEWS})")
                ...
                if max_posts and posts_count >= max_posts:
                    logger.info(f"الحد الأقصى: {max_posts}")
                    break

            if once and not news:
                logger.info("لا أخبار جديدة - خروج")
                break
    ...
    logger.info(f"ملخص: {posts_count} منشور")


def verify_sources():
    logger.info("فحص المصادر")
    working = 0
    total_iraqi = 0
    for source_url in NEWS_SOURCES:
        ...
        if status == 'ok':
            working += 1
            total_iraqi += iraqi
            logger.info(f"  {short}: {iraqi}/{total}")
        else:
            logger.info(f"  {short}: {status}")
    logger.info(f"النتيجة: {working}/{len(NEWS_SOURCES)} مصدر | {total_iraqi} خبر عراقي")


def show_startup_banner(once=False, verify_only=False):
    logger.info("=" * 50)
    logger.info("نظام نشر الأخبار - v26")
    if verify_only:
        logger.info("وضع: فحص المصادر")
    elif once:
        logger.info("وضع: دورة واحدة")
    else:
        logger.info("وضع: مستمر")
    logger.info(f"المصادر: {len(NEWS_SOURCES)}")
    logger.info(f"تصفية الصور: {REQUIRE_IMAGE}")
    logger.info("=" * 50)


def main():
    parser = argparse.ArgumentParser(description='نظام نشر الأخبار v26')
    ...
    while db_attempts < MAX_DB_INIT_ATTEMPTS:
        ...
        logger.error("فشل تهيئة DB")
        sys.exit(1)

    active_pages = [(k, p['name']) for k, p in PAGES_CONFIG.items() if p['token']]
    logger.info(f"بدء: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"صفحات مفعّلة ({len(active_pages)}): " + ', '.join(k for k, _ in active_pages))

    if not active_pages:
        logger.error("لا صفحات بتوكن")
        sys.exit(1)
